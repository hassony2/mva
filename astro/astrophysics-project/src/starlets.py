...
def starlet_transform(initialImage, wavelet_nb, filterName=h1d,
                      display=False, cmap='jet'):
    coeffs = np.zeros([wavelet_nb, initialImage.shape[0],
                       initialImage.shape[1]])
    meanWav = np.zeros(wavelet_nb)
    currentImage = cp.copy(initialImage)
    for iteration in range(wavelet_nb):
        ...
        if (display):
            ...
            plt.title('Starlet coeffs at scale {scale}'.format(
                scale=iteration + 1))
            plt.show()
    return coeffs, currentImage


def starlet_reconstruct(coeffs, final_coeffs):
    waveletNb = coeffs.shape[0]
    reconstruct_image = final_coeffs
    for idx in range(waveletNb):
        reconstruct_image = reconstruct_image + coeffs[idx]
