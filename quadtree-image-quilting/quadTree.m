function [ S, newImage, patches ] = quadTree( Image, styleImage, varThreshold, distThreshold, minDim, maxDim, padding, decal, vectorType )
...
%   @params threshold : max tolerance value withou split
    ...
    dim = maxDim;
    while (dim > minDim)
        patchIdQuadtree = zeros(M - 2*padding, M - 2*padding);
        ...
        for k=1:currentBlockNb
            ...
            if (stdDev < varThreshold)              
                if(not(computedPatches))
                    computedPatches = true;
                    patches = getPatches(styleImage, dim, padding, decal);
                end
                [ patch, normDist, fail, patchIdQuadtree] = getBestPatch( currentBlock, currentRow, currentCol, patches, vectorType, patchIdQuadtree, padding);
                dists(k)=normDist;
                if (normDist < distThreshold)
                    replaceImage = true;
                end
            end
